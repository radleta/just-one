...
`writeIdentityEvidence` restores the file's mtime via `utimesSync` afterwards: an older just-one sharing the same PID dir still verifies by mtime, and bumping it would make live processes look stale to that version. Pass float seconds, not `Date` objects — `Date` truncates to whole milliseconds. The restore is not bit-exact on any filesystem that stores a sub-microsecond mtime tail (APFS, ext4): float seconds near 1.8e9 exhaust a double's digits first, losing roughly 0.0005 ms. Both the macOS and the Linux e2e test assert the mtime survives to within 1 ms rather than exactly.
